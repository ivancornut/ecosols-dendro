...
<img width="729" height="968" alt="image" src="https://github.com/user-attachments/assets/dfafc157-46cb-4e3e-9f7b-13c80afe1ebb" />


## Objectives
Build a high precision point dendrometer to daily variations in water status and seasonnal growth of trees. 
There are already designs for point dendrometers on the market. Unfortunately most of them are based on a screw design and their linearity is sub-par or use very expensive LVDTs (100s of €). We choose to overcome these issues by inspiring ourselves on previous designs that use rods ([PepiPiaf](https://piaf.clermont.hub.inrae.fr/methodes-et-modeles/pepipiaf)) and by selecting high quality linear potentiometers instead (less than 50€).

## Hardware
### Mechanics
![Render of dendrometer](render_3D.png)

The build is based on a 3D printed holder and carbon-fiber rods. The 3D printed holder gives us the ease of fabrication, ease of prototyping and low-cost that makes it possible to build a large number of said dendrometers for a reasonnable price. They also make the design easily upgradeable with feedback from users. The carbon-fiber rods give us a good temperature response of the dendrometer due to their low expansion coefficient.
...
The linear potentiometer that we use has a stated 0.5% linearity (is usually better in practice) and includes the spring (assuring contact) inside its body. It comes in a practical package around which the 3D printed holder is designed. The reference is BT404 and it can be found [here](https://www.mouser.fr/ProductDetail/858-404R10KL.5).
#### ADC
...
## Calibration
### Fast Calibration of the linear potentiometer
To fast-calibrate the linearpotentiometer sensor we use a caliper and a small code that reads the outputs of the ADC connected to the linear potentiometer. The procedures, codes and example datasets to do this operation cab be found in the [/Calibration](/Calibration) folder. 
