...
module lvdt_BI404(rotation,offset_to_center,xscaling, zscaling,cylinder_scaling){
    ...
    color("gray")
    rotate([0,-rotation,0]){
        translate([26.92/2+5+offset_to_center+10/2,(12.7-7.88)/2,(11.56-8.51+1)/2+8.51-3.56]){
        // the space for wires to attach to the LVDT
            //cuboid([20+10, 5,11.56-8.51 +1], rounding = 1,edges=[TOP+FRONT,TOP+BACK],$fn=100);
            cuboid([20+10+10,(12.7-7.88)+xscaling,11.56-8.51 +1], rounding = 1,edges=[TOP+FRONT,TOP+BACK],$fn=100);
        }
        zscale(zscaling)
